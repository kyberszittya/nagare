...
\paragraph{Depth is triad-dominated; and the binary target does not reward
continuity.} Two calibrating measurements. (a) Multi-scale \emph{scalar} holonomy
$A^2\!+\!A^3\!+\!A^4$ lifts over triad-only $A^2$ only marginally and shrinking
with density (BTC-Alpha $+0.008$, Slashdot $+0.003$, Epinions $+0.001$): the
balance signal is largely triadic. (b) Signed hypergraphs are naturally
real-valued ($w_e\in[-1,1]$; holonomy $\prod w_e$ is a continuous product with
magnitude = confidence), and \texttt{clifford\_fir} models exactly that. Yet on
the rated Bitcoin graphs the \emph{weighted} $[-1,1]$ holonomy is \emph{worse}
than $\pm1$ for \emph{sign} prediction (BTC-Alpha $-0.006$, BTC-OTC $-0.013$) ---
because the target is binary, so binarising aligns with it and down-weighting
weak-but-correctly-signed edges loses votes. \textbf{Conclusion:} the continuous
holonomy's payoff is not on the binary sign benchmark (there $\pm1$ + triads
suffice); it is on a \emph{continuous target} --- predicting rating / trust
\emph{strength} --- where the $[-1,1]$ magnitude is necessary. That reframes the
decisive Nagare experiment.

\paragraph{Pure-Nagare result (Rust, closed-form, no backprop).}
\texttt{examples/signed\_link.rs} loads a real signed graph, computes leakage-free
triad-holonomy features, and trains a logistic with Nagare's \emph{local} update
rule ($W\mathrel{+}=-\eta(p-y)\phi$) on the shipped \texttt{linear\_forward}
kernel. Test AUROC: Bitcoin-Alpha $0.904$, Bitcoin-OTC $0.928$, Slashdot $0.910$,
Epinions $0.951$ --- matching the linear baseline and inside the published
SGCN/SiGAT band ($\sim0.93$--$0.97$). So Nagare's closed-form learner is
\emph{competitive} on real signed-link prediction; it matches rather than beats,
because the signal is triad-saturated --- the value is competitive quality at the
framework's speed/memory, not an accuracy breakout.
...
(4) Depth: \emph{scalar} deeper holonomy is triad-dominated, and \emph{weighted}
$[-1,1]$ holonomy does not beat $\pm1$ on the \emph{binary} sign target ---
\emph{measured}.
(5) Pure Nagare: the closed-form learner is \emph{competitive} ($0.90$--$0.95$,
matches baselines) on real signed-link prediction --- \emph{measured (Rust)}.
(6) \textbf{Reframed open}: the continuous holonomy that \texttt{clifford\_fir}
represents should pay off on a \emph{continuous} target --- predicting rating /
trust \emph{strength}, where the $[-1,1]$ magnitude is necessary --- not on binary
sign prediction. Rotor holonomy on strength regression is the remaining
differentiating experiment, \emph{not yet built}.
