...
\emph{measured}.
(5) \textbf{Reframed open}: the continuous holonomy that \texttt{clifford\_fir}
represents pays off on a \emph{continuous} target --- predicting rating / trust
\emph{strength}, where the $[-1,1]$ magnitude is necessary --- not on binary sign
prediction (there $\pm1$ + triads suffice). The decisive Nagare experiment is
therefore rotor holonomy on a strength-regression target, \emph{not yet built}.
