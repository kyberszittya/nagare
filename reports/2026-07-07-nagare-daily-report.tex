...
them, because on these datasets the signal is triad-saturated. \textbf{Caveat (from
prior in-house benchmarks):} on \emph{arity-2} signed graphs this AUROC is partly a
prevalence effect --- Bitcoin's $\sim$$90\%$ positive edges make a node-popularity
heuristic strong --- and signed-KAN architectures are only SGCN-competitive here.
The regime where they are measured to \emph{beat} graph GNNs (by $\sim$$+0.05$ AUC)
is \textbf{mixed-arity hypergraphs}, which graph-only models cannot represent. That
is the differentiating target going forward, and it is exactly what Nagare's
